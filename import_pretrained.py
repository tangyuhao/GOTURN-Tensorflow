...
for key in pretrained_weights.keys():
    if 'conv' in key:
        right_order_weights[key]['weights'] = np.transpose(pretrained_weights[key]['weights'],(2,3,1,0))
    else:
        right_order_weights[key]['weights'] = np.transpose(pretrained_weights[key]['weights'],(1,0))
