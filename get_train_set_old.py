...
# choose 1 frame from every K frames
K = 3
train_info_small = {}
for clip_path, boxes in train_info.items():
    train_info_small[clip_path] = [box for idx,box in enumerate(boxes) if idx % K == 1]
...
for clip_path, boxes in train_info_small.items():
    ...
    for curr_box in boxes:
        ...
        if curr_box[0] == prev_box[0] and int(curr_box[1][:6]) - K == int(prev_box[1][:6]):
